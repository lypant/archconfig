...
# TODO Consider running this script as a regular user - then add sudo where needed
...
#sudo
pacman -S --noconfirm vim alsa-utils xorg-server xorg-server-utils xorg-xinit mesa virtualbox-guest-utims dmenu rxvt-unicode ttf-inconsolata ttf-dejavu git
...
#sudo
sed -i 's/HOOKS="base udev autodetect modconf block filesystems keyboard fsck"/HOOKS="base ud    ev autodetect modconf block filesystems keyboard fsck consolefont keymap"/g' /etc/mkinitcpio.conf
